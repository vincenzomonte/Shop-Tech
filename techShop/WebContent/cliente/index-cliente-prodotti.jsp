...
<link rel='stylesheet' type='text/css'
	href='../css_<%=negozioBean.getColore() %>/sidenav-nav_left/stile-nav_left.css'>
<link rel='stylesheet' type='text/css'
	href='../css_<%=negozioBean.getColore() %>/sidenav-nav_left/sidenav-nav_left.css'>
<% }else if(negozioBean.getDesign().equals("top")){%>
<link rel='stylesheet' type='text/css'
	href='../css_<%=negozioBean.getColore() %>/sidenav-nav_top/stile-nav_top.css'>
<link rel='stylesheet' type='text/css'
	href='../css_<%=negozioBean.getColore() %>/sidenav-nav_top/sidenav-nav_top.css'>
<%} %>
<link rel='stylesheet' type='text/css'
	href='../css_<%=negozioBean.getColore() %>/step.css'>
<link rel='stylesheet' type='text/css'
	href='../css_<%=negozioBean.getColore() %>/popup.css'>
<link rel='stylesheet' type='text/css'
	href='../css_<%=negozioBean.getColore() %>/content.css'>
...
<body onresize='resize()' id='home'>
	...
	<header>
		...
		<div id='mySidenav' class="sidenav">
			<ul>
				...
				<li onclick="window.open('index-cliente.jsp','_self')">Negozi</li>
				<li
					onclick="window.open('index-cliente-categorie.jsp?negozio=<%=negozio %>','_self')">
					Categorie</li>
				<li><a href='../Logout'
	...
	<div class="content">
		...
		<h2>
			<a href='index-cliente.jsp'>Negozi</a>/<a
				href='index-cliente-categorie.jsp?negozio=<%=negozio %>'>Categorie</a>/Prodotti
		</h2>
				...
				<form action='' method='POST'>
					<div class="img">
						<img src="../<%= bean.getPath() %>"
							title="<%= bean.getDescrizione() %>">
					</div>
					<div class='nomeProdotto'>
						<span> <%= bean.getNome() %> <%= bean.getPrezzo() %>€
						</span>
					</div>
					<div class='note'>
						<span> <textarea id="note<%=count %>" placeholder='Note...'></textarea>
						</span>
					</div>
					<div class='qta'>

						<div class="quantity">
							<input type="number" min="0" id="quantita<%=count %>"
								max="<%= bean.getQuantita() %>" step="1" value="0">
						</div>
						<span>
							<button type='button' class='acquista-btn'
								onclick="addToCart('<%= negozio %>','<%=categoria%>',<%= bean.getIdProdotto() %>,<%=bean.getQuantita()%>,<%=count%>)">Carrello</button>
						</span>
					</div>
