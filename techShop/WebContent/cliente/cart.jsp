	...
	<div class="content">
		...
		<center>
			...
			<table style='max-width: 80%;'>
				...
				<tbody>
					...
					<%		
				                    ...
				                    %>
					<tr class="prod<%= count %>">
						<td data-label="Prodotto"><img width='90px'
							src='../<%=p.getPath() %>'><br><%=p.getNome() %></td>
						<td data-label="Prezzo"><%=p.getPrezzo() %>€</td>
						<td data-label="Quantità"><img src='../images/meno.png'
							onclick="minusProduct('<%= p.getNomeNegozio() %>','<%= p.getNomeCategoria() %>',<%= p.getIdProdotto() %>,<%=p.getQuantitaOrdinata()%>,'<%= count %>')"
							style='width: 0.5em; cursor: pointer;'> <span
							class='cartQta<%=count%>'><%=p.getQuantitaOrdinata() %> </span> <img
							src='../images/plus.png'
							onclick="addProduct('<%= p.getNomeNegozio() %>','<%= p.getNomeCategoria() %>',<%= p.getIdProdotto() %>,<%=p.getQuantita()%>,<%= count %>)"
							style='width: 0.5em; cursor: pointer;'></td>
						...
						<td data-label="Rimuovi"><img style='cursor: pointer;'
							width='40px'
							onclick="removeCartProduct('<%= p.getNomeNegozio() %>','<%= p.getNomeCategoria() %>',<%= p.getIdProdotto() %>,<%= count %>)"
			...
			<h2 style="display: inline; cursor: pointer;" onclick="confirm()">
				<span style="vertical-align: middle;">Conferma Ordine </span><img
